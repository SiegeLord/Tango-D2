...
char[] sub(char[] string, char[] pattern, char[] delegate(RegExp) dg, char[] attributes = null)
{
	...
	// Optimize by using phobos.string.replace if possible - Dave Fladebo
	    ...
	    pattern == slice)				// simple pattern (exact match, no special characters) 
	{
	    ...
	    result = phobos.string.replace(result,slice,replacement);
	    break;
	}
...
unittest
{
    ...
    auto s = sub("Strap a rocket engine on a chicken.",
		 "[ar]",
	         delegate char[] (RegExp m)
	         {
		    return phobos.string.toupper(m.match(0));
	         },
	         "g");
 ...
 * ---
 * auto s = "abcabcabab";
 * phobos.regexp.find(s, "b");    // match, returns 1
 * phobos.regexp.find(s, "f");    // no match, returns -1
 * ---
 */
 ...
 * ---
 * auto s = "abcabcabab";
 * phobos.regexp.find(s, "b");    // match, returns 9
 * phobos.regexp.find(s, "f");    // no match, returns -1
 * ---
 */
 ...
 * void main()
 * {
 *     if (auto m = phobos.regexp.search("abcdef", "c"))
 *     {
...
unittest
{
    ...
    if (auto m = phobos.regexp.search("abcdef", "c()"))
    {
	auto result = phobos.string.format("%s[%s]%s", m.pre, m.match(0), m.post);
    ...
    if (auto n = phobos.regexp.search("abcdef", "g"))
    {
	assert(0);
    }
}
...
class RegExp
{
	...
	foreach(m; RegExp("ab").search("abcabcabab"))
	{
	    auto s = phobos.string.format("%s[%s]%s", m.pre, m.match(0), m.post);
...
unittest
{
    ...
    assert(result.length == 2);
    i = phobos.string.cmp(result[0], "a");
    assert(i == 0);
    i = phobos.string.cmp(result[1], "b");
    assert(i == 0);

    r = new RegExp("a*", null);
    result = r.split("ab");
    assert(result.length == 2);
    i = phobos.string.cmp(result[0], "");
    assert(i == 0);
    i = phobos.string.cmp(result[1], "b");
    assert(i == 0);
    ...
    j = join(result, ",");
    //printf("j = '%.*s'\n", j);
    i = phobos.string.cmp(j, "a,,b,font,/,b,bar,,TAG,hello,/,TAG,");
    assert(i == 0);

    r = new RegExp("a[bc]", null);
    result = r.match("123ab");
    j = join(result, ",");
    i = phobos.string.cmp(j, "ab");
    assert(i == 0);
    
    result = r.match("ac");
    j = join(result, ",");
    i = phobos.string.cmp(j, "ac");
    assert(i == 0);
}
...
unittest
{
    ...
    i = phobos.string.cmp(result, "1xaby2xacy3");
    assert(i == 0);

    r = new RegExp("ab", "g");
    result = r.replace("1ab2ac3", "xy");
    i = phobos.string.cmp(result, "1xy2ac3");
    assert(i == 0);
}
...
{   int srcsave;
	...
	switch (program[pc])
	{
		...
		if (c1 != c2)
		{
			...
			c2 = phobos.ctype.toupper(cast(rchar)c2);
		    else
			goto Lnomatch;
		    if (c1 != c2)
			goto Lnomatch;
		...
		if (!(attributes & REA.dotmatchlf) && input[src] == cast(rchar)'\n')
		    goto Lnomatch;
		src += phobos.utf.stride(input, src);
		//src++;
		pc++;
		break;
...
{   rchar c;
	...
	switch (prog[i])
	{
		...
		if (c <= 0x7F)
		{   r.setbit2(c);
		    r.setbit2(phobos.ctype.tolower(cast(rchar)c));
		}
		return 1;
		...
		if (c <= 0x7F)
		{   r.setbit2(phobos.ctype.toupper(cast(rchar)c));
		    r.setbit2(phobos.ctype.tolower(cast(rchar)c));
		}
		return 1;
