...
char[] digestToString(ubyte[16] digest)
{
    ...
    foreach (ubyte u; digest)
    {
	result[i] = phobos.string.hexdigits[u >> 4];
	result[i + 1] = phobos.string.hexdigits[u & 15];
	i += 2;
    }
    return result;
}
...
struct MD5_CTX
{
	...
	*this = MD5_CTX.init;
    ...
    void update(void[] input)
    {
      ...
      if (inputLen >= partLen)
      {
	    phobos.c.string.memcpy(&buffer[index], input.ptr, partLen);
      ...
      if (inputLen - i)
	    phobos.c.string.memcpy(&buffer[index], &input[i], inputLen-i);
    }
    ...
    void finish(ubyte[16] digest)         /* message digest */
    {
      ...
      phobos.c.string.memset (this, 0, MD5_CTX.sizeof);
    }
...
unittest
{
    debug(md5) printf("phobos.md5.unittest\n");
