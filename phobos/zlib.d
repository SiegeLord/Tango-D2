...
private import phobos.etc.c.zlib;
...
{
    return phobos.etc.c.zlib.adler32(adler, cast(ubyte *)buf, buf.length);
}
...
{
    return phobos.etc.c.zlib.crc32(crc, cast(ubyte *)buf, buf.length);
}
...
body
{
    ...
    err = phobos.etc.c.zlib.compress2(cast(ubyte *)destbuf, &destlen, cast(ubyte *)srcbuf, srcbuf.length, level);
    if (err)
    {	delete destbuf;
...
void[] uncompress(void[] srcbuf, uint destlen = 0u, int winbits = 15)
{
	...
	err = phobos.etc.c.zlib.inflateInit2(&zs, winbits);
	if (err)
	{   delete destbuf;
	    throw new ZlibException(err);
	}
	err = phobos.etc.c.zlib.inflate(&zs, Z_NO_FLUSH);
	switch (err)
	{
	    case Z_OK:
		phobos.etc.c.zlib.inflateEnd(&zs);
		destlen = destbuf.length * 2;
		continue;

	    case Z_STREAM_END:
		destbuf.length = zs.total_out;
		err = phobos.etc.c.zlib.inflateEnd(&zs);
		if (err != Z_OK)
		    goto Lerr;
		return destbuf;
...
private import phobos.random;
private import tango.stdc.stdio;

unittest // by Dave
{
    debug(zlib) printf("phobos.zlib.unittest\n");

    bool CompressThenUncompress (ubyte[] src)
    {
      try {
	ubyte[] dst = cast(ubyte[])phobos.zlib.compress(cast(void[])src);
	...
	uncompressedBuf = cast(ubyte[])phobos.zlib.uncompress(cast(void[])dst);
    ...
    debug(zlib) printf("PASSED phobos.zlib.unittest\n");
}
