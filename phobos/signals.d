...
module phobos.signals;

import phobos.stdio;
import phobos.c.stdlib : calloc, realloc, free;
import phobos.outofmemory : _d_OutOfMemory;
...
template Signal(T1...)
{
	...
	if (slots_idx == len)
	{
	    ...
	    {
		len = 4;
		auto p = phobos.signals.calloc(slot_t.sizeof, len);
		if (!p)
		    phobos.signals._d_OutOfMemory();
		slots = (cast(slot_t*)p)[0 .. len];
	    }
	    else
	    {
		len = len * 2 + 4;
		auto p = phobos.signals.realloc(slots.ptr, slot_t.sizeof * len);
		if (!p)
		    phobos.signals._d_OutOfMemory();
		slots = (cast(slot_t*)p)[0 .. len];
