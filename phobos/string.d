...
private import phobos.uni;
private import phobos.array;
private import phobos.format;
private import phobos.ctype;
private import phobos.stdarg;
...
int ifind(char[] s, dchar c)
{
    ...
    if (c <= 0x7F)
    {	// Plain old ASCII
	char c1 = phobos.ctype.tolower(c);

	foreach (int i, char c2; s)
	{
	    c2 = phobos.ctype.tolower(c2);
	    if (c1 == c2)
		return i;
	}
    }
    ...
    {	// c is a universal character
	dchar c1 = phobos.uni.toUniLower(c);

	foreach (int i, dchar c2; s)
	{
	    c2 = phobos.uni.toUniLower(c2);
	    if (c1 == c2)
		return i;
	}
    }
...
int irfind(char[] s, dchar c)
{
	...
	{   char cx = s[i];
	    ...
	    size_t j = i;
	    dchar c2 = phobos.utf.decode(s, j);
	    c2 = phobos.uni.toUniLower(c2);
	    if (c1 == c2)
		break;
	}
...
int irfind(char[] s, char[] sub)
	...
	if (c <= 0x7F)
	{
	    c = phobos.ctype.tolower(c);
	    for (int i = s.length - sub.length; i >= 0; i--)
	    {
		if (phobos.ctype.tolower(s[i]) == c)
		{
...
{   auto len = s.length;
	...
	while ((s[len - 1] & 0xC0) == 0x80)
	{
	    len--;
	    if (len == 0)
		throw new phobos.utf.UtfException("invalid UTF sequence", 0);
	}
...
{   size_t i;
	...
	{
	    if (i >= s.length)
		throw new ArrayBoundsError("phobos.string.sformat", 0);
	    s[i] = c;
	    ++i;
	}
	else
...
unittest
{
    debug(string) printf("phobos.string.format.unittest\n");
...
unittest
{
    debug(string) printf("phobos.string.inPattern.unittest\n");
...
unittest
{
    debug(string) printf("phobos.string.count.unittest\n");
...
unittest
{
    debug(string) printf("phobos.string.remove.unittest\n");
...
unittest
{
    debug(string) printf("phobos.string.squeeze.unittest\n");
    char[] s,r;
...
unittest
{
    debug(string) printf("phobos.string.succ.unittest\n");
...
char[] tr(char[] str, char[] from, char[] to, char[] modifiers = null)
{
	...
	{   dchar t = phobos.utf.decode(to, i);
	    if (t == '-' && lastt != dchar.init && i < to.length)
	    {
		nextt = phobos.utf.decode(to, i);
...
unittest
{
    debug(string) printf("phobos.string.tr.unittest\n");
...
final bool isNumeric(in char[] s, in bool bAllowSep = false)
{
    ...
    bool   bComplex = false;
    char[] sx = phobos.string.tolower(s); 
    int    j  = 0;
    char   c;
...
bool isNumeric(TypeInfo[] _arguments, va_list _argptr)
{
    ...
    if (_arguments[0] == typeid(char[]))
        ...
        return isNumeric(phobos.utf.toUTF8(va_arg!(wchar[])(_argptr)));
    else if (_arguments[0] == typeid(dchar[]))
        return isNumeric(phobos.utf.toUTF8(va_arg!(dchar[])(_argptr)));
    ...
    else if (_arguments[0] == typeid(wchar))
    {
        ...
        return isNumeric(phobos.utf.toUTF8(ws));
    }
    else if (_arguments[0] == typeid(dchar))
    { 
        ds.length =  1;
        ds[0] = va_arg!(dchar)(_argptr);
        return isNumeric(phobos.utf.toUTF8(ds));
    }
 ...
 * 
 *    auto abbrevs = phobos.string.abbrev(list);
 * 
...
char[][char[]] abbrev(char[][] values)
{
	...
	for (size_t j = 0; j < value.length; j += phobos.utf.stride(value, j))
	{   char[] v = value[0 .. j];
