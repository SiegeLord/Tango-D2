...
class CFile : Stream {
  ...
  override char ungetc(char c) { 
    return cast(char)phobos.c.stdio.ungetc(c,cfile); 
  }
...
static this() {
  ...
  din = new CFile(phobos.c.stdio.stdin,FileMode.In);
  dout = new CFile(phobos.c.stdio.stdout,FileMode.Out);
  derr = new CFile(phobos.c.stdio.stderr,FileMode.Out);
}
