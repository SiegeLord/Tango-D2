...
interface InputStream {
   ...
   * and <a href="std_format.html">phobos.format</a>.
   *
...
interface OutputStream {
   ...
   * References: <a href="std_format.html">phobos.format</a>.
...
class Stream : InputStream, OutputStream {
  private import phobos.string, phobos.crc32, phobos.c.stdlib, phobos.c.stdio;
...
class File: Stream {
  ...
  void open(char[] filename, FileMode mode = FileMode.In) {
    ...
    writeable = cast(bool)(mode & FileMode.Out);
    version (Win32) {
      if (phobos.file.useWfuncs) {
	hFile = CreateFileW(phobos.utf.toUTF16z(filename), access, share,
			    null, createMode, 0, null);
      } else {
	hFile = CreateFileA(phobos.file.toMBSz(filename), access, share,
			    null, createMode, 0, null);
      }
  ...
  override void close() {
	...
	version (Win32) {
	  ...
	  phobos.c.linux.linux.close(hFile);
	  hFile = -1;
	}
      }
    }
  ...
  override size_t readBlock(void* buffer, size_t size) {
    ...
    version (Win32) {
      ...
      size = phobos.c.linux.linux.read(hFile, buffer, size);
      if (size == -1)
	size = 0;
    }
    readEOF = (size == 0);
  ...
  override size_t writeBlock(void* buffer, size_t size) {
    ...
    version (Win32) {
      ...
      size = phobos.c.linux.linux.write(hFile, buffer, size);
      if (size == -1)
	size = 0;
    }
    return size;
  ...
  unittest {
    ...
    assert(!phobos.string.cmp(line, "Testing stream.d:"));
    // jump over "Hello, "
    ...
    assert(!phobos.string.cmp(file.readString(6), "world!"));
    i = 0; file.read(i);
    assert(i == 666);
    // string#1 + string#2 + int should give exacly that
    version (Win32)
...
class BufferedFile: BufferedStream {
  ...
  unittest {
    ...
    assert(!phobos.string.cmp(file.readLine(), "Testing stream.d:"));
    // jump over "Hello, "
    file.seek(7, SeekPos.Current);
    version (Win32)
      assert(file.position() == 19 + 7);
...
class EndianStream : FilterStream {
   ...
   * in the phobos.system module.
   */
  this(Stream source, Endian end = phobos.system.endian) {
    super(source);
    endian = end;
  }
  ...
  final void fixBO(void* buffer, uint size) {
    if (endian != phobos.system.endian) {
  ...
  unittest {
    ...
    em.fixBO(x3,12);
    if (phobos.system.endian == Endian.LittleEndian) {
      assert( x3[0] == 12 );
      assert( x3[1] == 11 );
      assert( x3[2] == 10 );
      assert( x3[4] == 8 );
    ...
    em.position(0);
    em.fixBO(x3,12);
    if (phobos.system.endian == Endian.BigEndian) {
      assert( x3[0] == 12 );
      assert( x3[1] == 11 );
      assert( x3[2] == 10 );
      assert( x3[4] == 8 );
...
unittest {
  ...
  m.close();
  phobos.file.remove("testing.txt");
}
