...
public:
    int opCmp(Field rhs)
    {
        ...
        {
            res = phobos.string.cmp(m_name, rhs.m_name);

            if(0 == res)
            {
                res = phobos.string.cmp(m_value, rhs.m_value);
            }
        }
...
version(MainTest)
{
    ...
    int main(char[][] args)
    {
        ...
        else
        {
            ...
            try
            {
                printf( "phobos.openrj test:\n\tmodule:      \t%.*s\n\tdescription: \t%.*s\n\tversion:     \t%d.%d.%d.%d\n"
                    ,   phobos.openrj.VERSION.name
                    ,   phobos.openrj.VERSION.description
                    ,   phobos.openrj.VERSION.major
                    ,   phobos.openrj.VERSION.minor
                    ,   phobos.openrj.VERSION.revision
                    ,   phobos.openrj.VERSION.edit);

                counter.start();

                char[]      file        =   args[1];
                char[]      chars       =   cast(char[])phobos.file.read(file);
