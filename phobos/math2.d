...
module phobos.math2;
private import phobos.math, phobos.string, phobos.c.stdlib, phobos.c.stdio;
...
unittest
{
	assert(feq(tan(PI / 3), phobos.math.sqrt(3)));
}
+/
...
unittest
{
	assert(feq(cot(PI / 6), phobos.math.sqrt(3.0L)));
}
...
real acosh(real x)
{
	if (x <= 1)
		...
		return log(x + phobos.math.sqrt((x - 1) * (x + 1)));
}
...
real asinh(real x)
{
	...
	else
	{
		real z = x * x;
		return x > 0 ? 
			log1p(x + z / (1.0 + phobos.math.sqrt(1.0 + z))) :
			-log1p(-x + z / (1.0 + phobos.math.sqrt(1.0 + z)));
	}
}
