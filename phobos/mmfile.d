...
class MmFile
{
			...
			size_t window = 0)
    {
		...
		version (Win32)
		{
			...
			if (useWfuncs)
			{
				wchar* namez = phobos.utf.toUTF16z(filename);
			...
			{
				char* namez = phobos.file.toMBSz(filename);
		...
		else version (linux)
		{
			...
			{	
				...
				fd = phobos.c.linux.linux.open(namez, oflag, fmode);
				if (fd == -1)
				{
				...
				if (prot & PROT_WRITE && size > statbuf.st_size)
				{
					...
					phobos.c.linux.linux.lseek(fd, cast(int)(size - 1), SEEK_SET);
					char c = 0;
					phobos.c.linux.linux.write(fd, &c, 1);
				}
	...
	~this()
	{
		...
		{
			if (fd != -1 && phobos.c.linux.linux.close(fd) == -1)
				errNo();
			fd = -1;
		}
		else
...
unittest {
	...
	version(Win32) {
		/+ these aren't defined in phobos.c.windows.windows so let's use the default
         SYSTEM_INFO sysinfo;
