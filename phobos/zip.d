...
private import phobos.zlib;
private import phobos.date;
private import phobos.intrinsic;
...
class ArchiveMember
{
    ...
    phobos.date.DosFileTime time;	/// Read/Write: Last modified time of the member. It's in the DOS date/time format.
...
class ZipArchive
{
	...
	foreach (ArchiveMember de; directory)
	{
		...
		case 8:
		    de.compressedData = cast(ubyte[])phobos.zlib.compress(cast(void[])de.expandedData);
	    ...
	    de.crc32 = phobos.zlib.crc32(0, cast(void[])de.expandedData);
	...
	switch (de.compressionMethod)
	{
		...
		// and 4 byte trailer.
		de.expandedData = cast(ubyte[])phobos.zlib.uncompress(cast(void[])de.compressedData, de.expandedSize, -15);
		return de.expandedData;
