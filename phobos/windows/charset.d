 ...
 * functions and phobos.utf.toUTF16z and toUTF8, rather than toMBSz and
 * fromMBSz.
 */
int useWfuncs = 1;
...
char* toMBSz(char[] s, uint codePage = 0)
{
	...
	if (c >= 0x80)
	{
	    char[] result;
	    int readLen;
	    wchar* ws = phobos.utf.toUTF16z(s);
...
char[] fromMBSz(char* s, int codePage = 0)
{
	...
	if (*c >= 0x80)
	{
	    ...
	    return phobos.utf.toUTF8(result[0 .. result.length-1]); // omit trailing null
	}
    }
