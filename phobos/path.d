 ...
 * use the phobos.file module first (i.e. phobos.file.isDir()).
 */
...
version(linux)
{
    ...
    private import phobos.outofmemory;
...
version (Windows) alias phobos.string.cmp fcmp;

version (linux) alias phobos.string.icmp fcmp;
...
private char[] combineCPathWithDPath(char* c_path, char[] path, int char_pos)
{
    ...
    size_t end = phobos.string.strlen(c_path);
...
private char[] expandFromDatabase(char[] path)
{
    ...
    while (1)
    {
	extra_memory = phobos.c.stdlib.malloc(extra_memory_size);
	...
	phobos.c.stdlib.free(extra_memory);
	extra_memory_size *= 2;
    }
