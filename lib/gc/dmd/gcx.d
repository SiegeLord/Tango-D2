...
class GC
{
    ...
    void *malloc(size_t size, bool df = false)
    {	void *p;
	if (!thread_needLock())
	{
	     ...
	     */
	    return mallocNoSync(size, df);
	}
	else synchronized (gcLock)
	{
	    p = mallocNoSync(size, df);
	}
	return p;
    }

    void *mallocNoSync(size_t size, bool df = false)
    {   void *p = null;
	Bins bin;
    ...
    void *calloc(size_t size, bool df = false)
    {
	void *p;
    ...
    void *realloc(void *p, size_t size, bool df = false)
    {
	if (!size)
	{   if (p)
	    {   free(p);
