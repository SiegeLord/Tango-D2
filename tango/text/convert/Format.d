...
extern (C) private char* ecvt(double d, int digits, out int decpt, out bool sign);
        ...
        private static uint internalFormat(IFormatService formatService, char[] format, inout ArgumentIterator it, Sink sink)
        {
                ...
                char[] formatArgument(char[] format, inout Argument arg)
                {
                        switch (arg.getTypeCode())
                        {
                                ...
                                case TypeCode.UBYTE:
                                        return formatInteger(result, cast(long)*cast(ubyte*)arg.getValue(), format, formatService);

                                case TypeCode.BYTE:
                                        return formatInteger(result, cast(long)*cast(byte*)arg.getValue(), format, formatService);

                                case TypeCode.USHORT:
                                        return formatInteger(result, cast(long)*cast(ushort*)arg.getValue(), format, formatService);

                                case TypeCode.SHORT:
                                        return formatInteger(result, cast(long)*cast(short*)arg.getValue(), format, formatService);

                                case TypeCode.UINT:
                                        return formatInteger(result, cast(long)*cast(uint*)arg.getValue(), format, formatService);

                                case TypeCode.INT:
                                        return formatInteger(result, cast(long)*cast(int*)arg.getValue(), format, formatService);

                                case TypeCode.ULONG:
                                        return formatInteger(result, cast(long)*cast(ulong*)arg.getValue(), format, formatService);

                                case TypeCode.LONG:
                                        return formatInteger(result, *cast(long*)arg.getValue(), format, formatService);

                                        version (mlfp)
                                case TypeCode.FLOAT:
                                        return formatDouble(result, cast(double)*cast(float*)arg.getValue(), format, formatService);

                                        version (mlfp)
                                case TypeCode.DOUBLE:
                                        return formatDouble(result, *cast(double*)arg.getValue(), format, formatService);
...
private void formatFixed(inout Number number, inout Result target, int length, int[] groupSizes, char[] decimalSeparator, char[] groupSeparator)
{
        int pos = number.scale;
        char* p = number.digits;
