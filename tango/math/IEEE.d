...
private:
    @property static IeeeFlags getIeeeFlags()
    {
...
unittest {
    ...
    assert(ilogb(-0.0) == FP_ILOGB0);
    // denormal
    assert(ilogb(0.125 * real.min_normal) == real.min_exp - 4);
...
unittest
{
    ...
    assert(isZero(-0.0));
    assert(!isZero(2.5));
    assert(!isZero(real.min_normal / 1000));
}
}
...
unittest {
 static if (real.mant_dig == 64) {
  ...
  // Tests for 80-bit reals
    ...
    assert( nextUp(-0.0) == real.min_normal*real.epsilon );
    assert( nextUp(0.0) == real.min_normal*real.epsilon );
    ...
    assert( nextDoubleUp(-double.min_normal) == -double.min_normal*(1-double.epsilon) );
    assert( nextDoubleUp(-double.min_normal*(1-double.epsilon) == -double.min_normal*(1-2*double.epsilon)) );
    assert( isIdentical(-0.0, nextDoubleUp(-double.min_normal*double.epsilon)) );
    assert( nextDoubleUp(0.0) == double.min_normal*double.epsilon );
    assert( nextDoubleUp(-0.0) == double.min_normal*double.epsilon );
    assert( nextDoubleUp(double.min_normal*(1-double.epsilon)) == double.min_normal );
    assert( nextDoubleUp(double.min_normal) == double.min_normal*(1+double.epsilon) );
    // positive numbers
    ...
    assert( nextFloatUp(-float.min_normal) == -float.min_normal*(1-float.epsilon) );
    assert( nextFloatUp(1.0) == 1.0+float.epsilon );
    assert( nextFloatUp(-0.0) == float.min_normal*float.epsilon);
...
unittest {
    ...
    assert(ieeeMean(0.5*real.min_normal*(1-4*real.epsilon),0.5*real.min_normal)==0.5*real.min_normal*(1-2*real.epsilon));
}
}
