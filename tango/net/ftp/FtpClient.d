...
class FTPConnection: Telnet {
	...
	protected Time parseTimeval(char[] timeval) {
		...
		return Gregorian.generic.toTime(Integer.atoi(timeval[0 .. 4]),
				Integer.atoi(timeval[4 .. 6]), Integer.atoi(timeval[6 .. 8]),
	...
	protected SocketConduit getDataSocket() {
		...
		switch(this.inf_.type) {
			...
			case FtpConnectionType.active:
				...
				// Start listening.
				SocketConduit listener = new SocketConduit();
				...
				if(this.is_supported("EPRT")) {
					...
					auto str = Text.layout(tmp, "%0,%1,%2,%3,%4,%5",
							Integer.format(foo[0 .. 3], h1), Integer.format(
									foo[3 .. 6], h2), Integer.format(
									foo[6 .. 9], h3), Integer.format(
									foo[9 .. 12], h4), Integer.format(
									foo[12 .. 15], p1), Integer.format(
									foo[15 .. 18], p2));
	...
	public SocketConduit connectPassive() {
		...
		// Extended passive mode (IP v6, etc.)
			...
			char[]
					address = r.match(1) ~ "." ~ r.match(2) ~ "." ~ r.match(3) ~ "." ~ r.match(
							4);
			uint
					port = (((cast(int) Integer.parse(r.match(5))) << 8) + (r.match(
							7).length > 0 ? cast(int) Integer.parse(r.match(7)) : 0));
		...
		SocketConduit sock = new SocketConduit();
		...
		switch(this.inf_.type) {
			...
			case FtpConnectionType.active:
				...
				while(Clock.now < end_time) {
					...
					new_data = data.socket.accept();
					break;
				}

				if(new_data is null)
					throw new FtpException("CLIENT: No connection from server",
							"420");
	...
	/*****************************************************************************
	 Changed Since: 0.99.8
	 *****************************************************************************/
	public void finishDataCommand(SocketConduit data) {
	...
	/*****************************************************************************
	 Changed Since: 0.99.8
	 *****************************************************************************/
	public SocketConduit processDataCommand(char[] command, char[][] parameters...) {
		// Create a connection.
		SocketConduit data = this.getDataSocket();
		scope(failure) {
	...
	/*****************************************************************************
	 Changed Since: 0.99.8
	 *****************************************************************************/
	protected void readStream(SocketConduit data, OutputStream stream,
	...
	/*****************************************************************************
	 Changed Since: 0.99.8
	 *****************************************************************************/
	protected void sendStream(SocketConduit data, InputStream stream,
			FtpProgress progress = null)
	in {
		assert(data !is null);
		assert(stream !is null);
	...
	body {
		...
		SocketConduit data = this.processDataCommand("STOR", path);
	...
	body {
		...
		// Okay server, we want to store something...
		SocketConduit data = this.processDataCommand("APPE", path);
	...
	body {
		...
		SocketConduit data = this.processDataCommand("RETR", path);
