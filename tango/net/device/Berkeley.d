...
version (Win32)
{
        ...
        package extern (Windows)
        {
                ...
                int send(const(socket_t) s, const(void)* buf, int len, int flags);
                int sendto(const(socket_t) s, const(void)* buf, int len, int flags, Address.sockaddr* to, int tolen);
        ...
        static this()
        {
                lib = LoadLibraryA(toStringz("Ws2_32.dll"));
